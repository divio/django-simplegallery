...
class Migration:
    ...
    no_dry_run = True # We're doing ORM magic, so no dry run please
    
    def forwards(self, orm):
        
        # Adding field 'Gallery.name'
        db.add_column('simplegallery_gallery', 'name', orm['simplegallery.gallery:name'])
        for gallery in orm.Gallery.objects.all():
            gallery_title = gallery.translations.all()[0].title
            if gallery_title:
                if not orm.Gallery.objects.filter(name=gallery_title).count():
                    gallery.name = gallery_title
                else:
                    i = 1
                    newname = '%s-%s' % (gallery_title, i)
                    while orm.Gallery.objects.filter(name=newname).count():
                        i += 1
                        newname = '%s-%s' % (gallery_title, i)
