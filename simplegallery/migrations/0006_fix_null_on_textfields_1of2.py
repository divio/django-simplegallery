...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        '''
        Empty CharFields and TextFields should never be 'NULL', 
        they should be an empty string ('').
        '''
        for t in orm['simplegallery.gallerytranslation'].objects.all():
            if not t.description:
                t.description = ''
            if not t.title:
                t.title = ''
            t.save()
        for t in orm['simplegallery.imagetranslation'].objects.all():
            if not t.description:
                t.description = ''
            if not t.title:
                t.title = ''
